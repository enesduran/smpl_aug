...
class DFaustDataset(torch.utils.data.Dataset):
    def __init__(self, data_path, train_flag, setting_name='kinect', gt_flag=False, aug_flag=False):
        ...
        self.dataloader_path = f'smpl_net/data/{self.train_flag}_gt_{gt_flag}_aug_{aug_flag}.npz'
    ...
    def load_data(self):
        return np.load(self.dataloader_path, allow_pickle=True).item()
    ...
    def save_data(self):
        ...
        for key, value in self.data_dict.items():
            if (value['gt_flag'] == self.gt_flag):
                if not self.aug_flag and (value['aug_flag'] == self.aug_flag):
                    continue
                else:
